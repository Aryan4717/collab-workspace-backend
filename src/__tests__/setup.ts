...
jest.mock('../shared/utils/cache.util', () => ({
  CacheService: {
    ...
    projectListKey: jest.fn(
      workspaceId => `project:workspace:${workspaceId}:list`
    ),
    memberRoleKey: jest.fn(
      (workspaceId, userId) => `member:role:${workspaceId}:${userId}`
    ),
    permissionKey: jest.fn(
